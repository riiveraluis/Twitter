...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: Secret.consumerKey, consumerSecret: Secret.consumerSecret)
    var loginSuccess: (() -> ())?
    ...
    func getProfileDetails(success: @escaping ([NSDictionary]) -> (), failure: @escaping (Error) -> ()) {
    ...
    func postTweet(_ tweetContent: String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/statuses/update.json"
        
        TwitterAPICaller.client?.post(url, parameters: ["status": tweetContent], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
